...
public class SQL_Cart {
	...
	public static final String SELECT_CART_PLUS ="UPDATE `km_product_cart` SET `count`=`count`+? WHERE cartNo=(SELECT * FROM(SELECT cartNo FROM `km_product_cart` WHERE `prodNo`=? and`uid`=?) temp_1);";
